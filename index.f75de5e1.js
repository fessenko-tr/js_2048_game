...
function rearrangeCells(direction) {
    ...
    const initialRowValues = rows.map((row)=>[
            ...row.children
    ...
    rows.forEach((row)=>{
        ...
        const filledCellsValues = initialCells.filter((cell)=>cell.textContent !== "").map(({ textContent })=>textContent);
    ...
    const finalRowValues = rows.map((row)=>[
            ...row.children
        ].map((cel)=>cel.textContent)).flat();
...
function hasPossibleMoves() {
    ...
    for(let i = 0; i < fieldRows.length; i++)for(let i2 = 0; i2 < fieldRows[i].children.length; i2++){
